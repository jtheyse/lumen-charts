...
4. Profiled in September 2026; the measurements are in [PERFORMANCE.md](PERFORMANCE.md). A second renderer is not justified on that evidence. The families expected to need one are flat, because sampling caps their rendered marks: a line chart costs the same in the browser at 100,000 input points as at 1,000. The families that do degrade are the unsampled point clouds, where the cost is DOM size rather than drawing, and the cheaper answer is aggregation rather than a Canvas renderer that would duplicate every axis, tooltip and accessibility affordance. Density binning for scatter and bubble is the open item.
