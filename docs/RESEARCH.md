...
4. Profiled in September 2026; the measurements are in [PERFORMANCE.md](PERFORMANCE.md). A second renderer is not justified on that evidence. The families expected to need one are flat, because sampling caps their rendered marks: a line chart costs the same in the browser at 100,000 input points as at 1,000. The families that do degrade are the unsampled point clouds, where the cost is DOM size rather than drawing, and the cheaper answer is aggregation rather than a Canvas renderer that would duplicate every axis, tooltip and accessibility affordance. Density binning shipped for scatter in 0.7.0; bubble keeps every point, because binning would destroy its size encoding.
5. Finish the verification work. An accessibility pass with contrast, role, name and tab-order assertions shipped in 0.6.0, together with a WebAssembly sample host that is exercised in a browser; building it exposed a framework-reference defect that had made the component library unusable in WebAssembly. Browser automation followed in 0.6.1: a Playwright suite drives both hosts in continuous integration and immediately found a clipped-mark defect, and an axe-core sweep of the WCAG A and AA rules runs with it, which found colour-contrast failures throughout the samples. Screen-reader runs remain open, as does the majority of the WCAG criteria that no automated tool can judge.
